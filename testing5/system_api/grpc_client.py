...
# Add path to include generated gRPC code
...
# Post service URL from environment variable
...
class PostServiceClient:
    ...
    def update_post(self, post_id: int, creator_id: int, title: Optional[str] = None,
        ...
        try:
            # Попробуем получить текущий пост
            try:
                current_post = self.get_post(post_id, creator_id)
            except grpc.RpcError:
                # Если не удалось получить текущий пост, используем пустые значения
                current_post = {
                    "title": "",
                    "description": "",
                    "is_private": False,
            ...
            request = posts_pb2.UpdatePostRequest(
                ...
                tags=request_tags or []  # Защищаемся от None
            )
