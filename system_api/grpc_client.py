...
class PostServiceClient:
    ...
    def like_post(self, post_id: int, user_id: int):
        logger.info(f"Liking post with ID: {post_id}, user_id: {user_id}")
        try:
            request = posts_pb2.LikePostRequest(post_id=post_id, user_id=user_id)
            response = self.stub.LikePost(request)
            return {
                "success": response.success,
                "like_id": response.like_id if response.success else None
            }
        except grpc.RpcError as e:
            status_code = e.code()
            details = e.details()
    ...
    def view_post(self, post_id: int, user_id: int):
        logger.info(f"Viewing post with ID: {post_id}, user_id: {user_id}")
        try:
            request = posts_pb2.ViewPostRequest(post_id=post_id, user_id=user_id)
            response = self.stub.ViewPost(request)
            return {
                "success": response.success,
                "view_id": response.view_id if response.success else None
            }
        except grpc.RpcError as e:
            status_code = e.code()
            details = e.details()
    ...
    def add_comment(self, post_id: int, user_id: int, text: str):
        logger.info(f"Adding comment to post with ID: {post_id}, user_id: {user_id}")
        try:
            request = posts_pb2.AddCommentRequest(post_id=post_id, user_id=user_id, text=text)
            response = self.stub.AddComment(request)
            if not response.id:
    ...
    def get_post_comments(self, post_id: int, user_id: int, page: int = 1, page_size: int = 10):
        logger.info(f"Getting comments for post with ID: {post_id}, user_id: {user_id}")
        try:
            ...
            response = self.stub.GetPostComments(request)
